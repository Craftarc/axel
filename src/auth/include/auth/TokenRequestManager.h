...
namespace auth {
    ...
    class TokenRequestManager : public ITokenRequestManager {
            /// @brief Responsible for constructing and storing the POST
            /// request for the access token exchange.
        ...
        public:
            ...
            [[nodiscard]]
            std::string send_token_request(std::string auth_code,
                                           std::string code_verifier,
                                           std::shared_ptr<util::IHttpSender>
                                           http_sender) const override;
    };
}  // namespace auth
