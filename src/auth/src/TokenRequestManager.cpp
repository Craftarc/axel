...
std::string auth::TokenRequestManager::send_token_request(std::string auth_code,
	...
	{ { "client_id", config::axel::client_id },
	  ...
	  { "scope",
		...
		{ scopes::profile, scopes::stashes, scopes::characters, scopes::item_filter, scopes::league_accounts }) },
	  { "code_verifier", std::move(code_verifier) } });

	// Add the headers to the request
	auto full_request = util::make_http_request(
	"POST",
	config::poe::paths::token,
	{ { "host", config::poe::host }, { "content-type", "application/x-www-form-urlencoded" } },
	request_body);
