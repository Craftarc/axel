...
namespace axel {
    ...
    protected:
        ...
        int setup() {
            EXPECT_CALL(*mock_auth_client, DescribeTable(_)).Times(1)
    ...
    TEST_F(Database_test, put__error_if_partition_key_value_not_in_list_of_items_to_add_to_table) {
        ...
        std::unordered_map<Aws::String, Aws::String> items{{{"state_hash", ""},
                                                            {"code_verifier", ""},
                                                            {"time_to_live", ""}}};
    ...
    TEST_F(Database_test, put__valid_if_partition_key_value_in_list_of_items_to_add_to_table) {
        ...
        std::unordered_map<Aws::String, Aws::String> items{{{"session_id", ""},
                                                            {"state_hash", ""},
                                                            {"code_verifier", ""},
                                                            {"time_to_live", ""}}};
