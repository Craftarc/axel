...
namespace axel {
    ...
    class PlayerItems {
        ...
        public:
            ...
            const std::unordered_map<std::string, int64_t>& get_update();

        protected:
            /// @brief Constructor. Allows the usage of a custom HttpSender.
            PlayerItems(const std::string& access_token, std::shared_ptr<util::IHttpSender> http_sender);
        ...
        private:
            ...
            /// @brief Input sanitiser for PlayerItems.
            static int sanitise(const std::string& access_token);
    };
}  // namespace axel
