...
namespace parse {
	...
	class JsonResult {
		public:
			...
			/// @param result Result of the search, in STL types or primitives (instead of boost::json::value).
			/// @param ec Error code. The operation failed if ec != 0.

			JsonResult(T result, int ec) : result_(std::move(result)), success_{ ec == 0 } {
				static_assert(is_json_type<T>::value, "Template argument must be a Boost::JSON type");
			}
			...
			/// @brief Gets the JSON result in the correct type (array, object, etc.) instead of value
			const T& get() {
				return result_;
			}
